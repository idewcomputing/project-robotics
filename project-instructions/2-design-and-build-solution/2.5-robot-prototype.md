...
4. **Use the method\(s\) described in your robot specifications to measure or evaluate your robot's task performance \(such as:  accuracy, efficiency, avoiding errors, etc.\).**
   ...
   * Once you are satisfied with the robot's performance \(or cannot improve it further\), perform 3-5 final trials to test the robot, and create a table to report the results for each trial, as well as the average performance across the trials.
